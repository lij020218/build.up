...
--
--  ⚠️ 각 문장은 CTE 로 자기완결(임시 테이블 미사용). Supabase SQL 러너/에디터가 문장을
--     개별 세션으로 분할 실행해도 안전(임시테이블 42P01 회피). 멱등(재실행 안전).
-- ════════════════════════════════════════════════════════════════════════
...
ranked as (
  select sd.id,
    row_number() over (
      partition by m.user_id, sd.stage_code
      order by sd.completed_at desc nulls last, sd.updated_at desc nulls last, sd.id desc
    ) as rn
  from public.stage_decisions sd
  join rmap m on m.roadmap_id = sd.roadmap_id
)
...
set roadmap_id = rmap.canon_id
from rmap
where d.roadmap_id = rmap.roadmap_id
  and d.roadmap_id <> rmap.canon_id;
...
ranked as (
  select st.id,
    row_number() over (
      partition by m.user_id, st.stage_code, st.task_code
      order by st.updated_at desc nulls last, st.id desc
    ) as rn
  from public.stage_tasks st
  join rmap m on m.roadmap_id = st.roadmap_id
)
...
set roadmap_id = rmap.canon_id
from rmap
where t.roadmap_id = rmap.roadmap_id
  and t.roadmap_id <> rmap.canon_id;
...
using canon
where r.user_id = canon.user_id
  and r.id <> canon.roadmap_id;
