...
export async function exportComprehensiveExcel(params: {
  ...
  const csv = sections
    ...
    .join("\n\n");
  const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8;" });
  triggerDownload(blob, `foundone_경영보고서_${month}.csv`);
}
...
export async function exportMonthlyPnLPdf(params: {
  ...
  doc.text(`Generated: ${getKstDate(new Date())} | Found.One`, 40, y);
  y += 26;
  ...
  doc.text("Generated by Found.One — roadmap-first startup companion", 40, 810);

  // 저장
  doc.save(`foundone_PnL_${month}.pdf`);
}
