...
import { Users, ShieldCheck, Link2, Copy, Check as CheckIcon } from "lucide-react";
 ...
 */
function InviteLinkSection({ ko }: { ko: boolean }) {
  ...
  return (
    ...
    >
      ...
      {status !== "ready" && (
        ...
        >
          {status === "loading" ? (ko ? "생성 중…" : "Creating…") : ko ? "+ 새 초대 링크 생성" : "+ Generate invite link"}
        </button>
      )}
      ...
      {status === "ready" && code && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <button
            type="button"
            onClick={() => { setCode(null); setStatus("idle"); setCopied(false); }}
          ...
          >
            {ko ? "다른 직원용 새 링크 생성" : "Create another"}
          </button>
        </div>
      )}
    </div>
