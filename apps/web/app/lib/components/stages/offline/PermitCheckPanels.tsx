...
export function PermitCheckPanels() {
  ...
  const negotiateFavorable: Record<string, { context: string; recommendation: string; rationale: string }> = {
    ...
    beauty: { context: "미용 / 뷰티", recommendation: "면허자 의존 X(본인 취득·2명+) · 배수·온수 부족은 임대인 분담 특약",
    ...
    fitness: { context: "필라테스·요가·PT", recommendation: "층고 2.5m 이하·바닥 하중 부족은 패스",
      rationale: "PT·필라 3m+, 요가 2.7m+ 권장. 바닥 보강은 2,000만원+ — 협상보다 매물 변경이 빠름." },
    education: { context: "학원 / 교육", recommendation: "건축물 용도 「교육연구시설」·「근린생활시설(학원)」 + 100㎡↑ 소방완비",
