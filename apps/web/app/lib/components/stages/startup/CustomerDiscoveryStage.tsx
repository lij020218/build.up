    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "14px", marginBottom: "14px" }}>
      ...
      <div style={{ borderRadius: "20px", border: "1px solid rgba(25,25,112,0.08)", background: "white", overflow: "hidden" }}>
        ...
        {guideSelections["interview-result"] && (() => {
          ...
          const downloadPdf = () => {
            // 2026-07-15 보안(XSS): window.open("")+document.write 는 같은 origin 문서를 만든다.
            //   script.* 는 LLM 이 사장님 입력을 반영해 생성한 JSON(공유 로드맵 상태) → 이스케이프
            //   없이 넣으면 &lt;img onerror&gt; 등이 실행돼 localStorage 세션 토큰 탈취 가능. 전 필드 이스케이프.
            const esc = (v: unknown) => String(v ?? "")
              .replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")
              .replace(/"/g, "&quot;").replace(/'/g, "&#39;");
            const printContent = `
              <html><head><meta charset="utf-8"><title>${esc(script.title)}</title>
              ...
              <p style="color:#666">${esc(script.duration)} · Mom Test 기반</p>
              <h2>핵심 원칙</h2>
              ${script.principles?.map((p: string) => `<p>• ${esc(p)}</p>`).join("") ?? ""}
              <h2>아이스브레이커</h2><p>${esc(script.icebreaker)}</p>
              <h2>질문</h2>
              ${script.questions?.map((q: {phase:string;question:string;purpose:string;followUp?:string}, i: number) =>
                `<div class="q"><span class="pill">${esc(q.phase)}</span><div class="q-text">${i+1}. ${esc(q.question)}</div><div class="q-purpose">${esc(q.purpose)}</div>${q.followUp ? `<div class="q-purpose">${esc(q.followUp)}</div>` : ""}</div>`
              ).join("") ?? ""}
              <h2>마무리</h2><p>${esc(script.closing)}</p>
              <h2>하지 말 것</h2>
              ${script.doNots?.map((dn: string) => `<p class="donot">${esc(dn)}</p>`).join("") ?? ""}
