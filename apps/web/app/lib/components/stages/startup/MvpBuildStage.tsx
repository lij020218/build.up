  ...
  const toolCard = (tool: MvpTool, color: string) => (
    <a key={tool.name} href={tool.url} target="_blank" rel="noreferrer" style={{ display: "flex", gap: "10px", padding: "10px 12px", borderRadius: "12px", background: `${color}04`, border: `1px solid ${color}10`, textDecoration: "none", color: "inherit" }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: "11px", color: "var(--muted)", lineHeight: 1.4, marginTop: "2px" }}>{tool.desc}</div>
      </div>
  ...
  const pages = ko ? [
    ...
    { title: "MVP 구축 로드맵", color: MIDNIGHT, content: (
      <div>
        ...
        <div style={{ fontSize: "13px", color: "var(--muted)", lineHeight: 1.7, marginBottom: "14px" }}>Reid Hoffman: "첫 버전이 창피하지 않다면 너무 늦게 출시한 것이다." Stripe는 7줄의 코드로 시작했습니다. LinkedIn은 런칭 직전 팀이 "Contact Finder를 먼저 만들자"고 했지만 Hoffman은 거절했고 — 7년이 지나도 그 기능은 필요 없었습니다.</div>
        ...
        <div style={{ padding: "12px 14px", borderRadius: "12px", background: "rgba(25,25,112,0.04)", border: "1px solid rgba(25,25,112,0.12)", marginBottom: "14px" }}>
          ...
          <div style={{ fontSize: "11.5px", color: "var(--muted)", marginTop: "8px", lineHeight: 1.5, fontStyle: "italic" as const }}>
            ...
            {[
              ...
              <div key={s.step} style={{ display: "flex", gap: "8px", alignItems: "flex-start", padding: "6px 0" }}>
                ...
                <div>
                  ...
                  <span style={{ fontSize: "11px", color: "var(--muted)", marginLeft: "4px" }}>— {s.desc}</span>
                </div>
              </div>
            ))}
          </div>
...
`}
        ...
        <div style={{ marginTop: "20px", padding: "16px 18px", borderRadius: "16px", background: `${MIDNIGHT}06`, border: `1.5px solid ${MIDNIGHT}30` }}>
          ...
          <div style={{ marginTop: "12px", padding: "10px 12px", borderRadius: "10px", background: "rgba(15,23,42,0.03)", border: "1px solid rgba(15,23,42,0.06)" }}>
            <div style={{ fontSize: "11.5px", fontWeight: 700, color: "var(--muted)", letterSpacing: "0.04em", marginBottom: "4px" }}>⚠ 흔한 함정</div>
    ...
    { title: "MVP Build Roadmap", color: MIDNIGHT, content: (<div><div style={{ fontSize: "15px", fontWeight: 680, color: "#0f172a", lineHeight: 1.5, marginBottom: "10px" }}>Ship a product that solves one core workflow in 2-6 weeks.</div><div style={{ fontSize: "13px", color: "var(--muted)", lineHeight: 1.7 }}>Reid Hoffman: "If you're not embarrassed by the first version, you've launched too late." Navigate through 7 steps using the arrows below.</div></div>) },
    { title: "Name & Mission", color: MIDNIGHT, content: (<div><div style={{ fontSize: "13px", color: "var(--muted)", lineHeight: 1.7, marginBottom: "14px" }}>Explain what your product does in one sentence. This becomes your landing page headline, your first pitch to investors, and your team's north star.</div><div style={{ display: "grid", gap: "6px" }}>{toolCard({ name: "Namelix", desc: "AI brand names + domain check", url: "https://namelix.com", free: true }, MIDNIGHT)}{toolCard({ name: "Claude / ChatGPT", desc: "Mission, slogan, elevator pitch", url: "https://claude.ai", free: true, tag: "AI" }, MIDNIGHT)}</div></div>) },
    { title: "Core Workflow & Wireframe", color: MIDNIGHT, content: (<div><div style={{ fontSize: "13px", color: "var(--muted)", lineHeight: 1.7, marginBottom: "14px" }}>Design the minimum path from signup to "aha moment." One workflow, perfect. Paul Graham: "Build something a small number of people want a large amount."</div><div style={{ display: "grid", gap: "6px" }}>{toolCard({ name: "Google Stitch", desc: "Text → UI design. 350 free/mo", url: "https://stitch.withgoogle.com", free: true, tag: "Best" }, MIDNIGHT)}{toolCard({ name: "v0 by Vercel", desc: "Prompt → production React+Tailwind", url: "https://v0.app", free: true }, MIDNIGHT)}</div></div>) },
    { title: "Architecture & DB Design", color: MIDNIGHT, content: (<div><div style={{ fontSize: "13px", color: "var(--muted)", lineHeight: 1.7, marginBottom: "14px" }}>Design the full structure before writing code. DB tables, API routes, page structure, auth flow. Skip this and you'll rebuild from scratch later.</div><div style={{ display: "grid", gap: "6px" }}>{toolCard({ name: "Eraser.io", desc: "AI architecture + ERD diagrams", url: "https://eraser.io", free: true }, MIDNIGHT)}{toolCard({ name: "Supabase Studio", desc: "Visual table editor + schema", url: "https://supabase.com", free: true }, MIDNIGHT)}</div></div>) },
    { title: "Backend & Deployment", color: MIDNIGHT, content: (<div><div style={{ fontSize: "13px", color: "var(--muted)", lineHeight: 1.7, marginBottom: "14px" }}>Don't manage servers. Use BaaS for auth, DB, storage in a few lines. 50%+ of YC startups use React, 25.6% deploy on Vercel.</div><div style={{ display: "grid", gap: "6px" }}>{toolCard({ name: "Supabase", desc: "PostgreSQL + Auth + Storage. Free 50K MAU", url: "https://supabase.com", free: true, tag: "DB" }, MIDNIGHT)}{toolCard({ name: "Vercel", desc: "Zero-config Next.js deploy", url: "https://vercel.com", free: true, tag: "Deploy" }, MIDNIGHT)}</div></div>) },
    { title: "Code with AI", color: MIDNIGHT, content: (<div><div style={{ fontSize: "13px", color: "var(--muted)", lineHeight: 1.7, marginBottom: "14px" }}>One person with AI tools = 5-person team. Strategy: Lovable for prototype (hours) → Cursor + Claude Code for production (2-4 weeks).</div><div style={{ display: "grid", gap: "6px" }}>{toolCard({ name: "Cursor", desc: "Fastest AI coding. VS Code fork", url: "https://cursor.com", free: false, tag: "Speed $20/mo" }, MIDNIGHT)}{toolCard({ name: "Claude Code", desc: "Best quality. 5.5x fewer tokens", url: "https://claude.ai/code", free: false, tag: "Quality" }, MIDNIGHT)}{toolCard({ name: "Lovable", desc: "Chat-to-app. One-click deploy", url: "https://lovable.dev", free: true, tag: "Prototype" }, MIDNIGHT)}</div></div>) },
    { title: "Design & Branding", color: MIDNIGHT, content: (<div><div style={{ fontSize: "13px", color: "var(--muted)", lineHeight: 1.7, marginBottom: "14px" }}>Even an MVP needs good design. Users judge by feel first. shadcn/ui gives Apple-level components for free.</div><div style={{ display: "grid", gap: "6px" }}>{toolCard({ name: "shadcn/ui", desc: "2026 standard. Tailwind + Radix", url: "https://ui.shadcn.com", free: true, tag: "Must" }, MIDNIGHT)}{toolCard({ name: "Looka", desc: "AI logo + brand kit", url: "https://looka.com", free: false, tag: "$20~" }, MIDNIGHT)}</div></div>) },
    { title: "Landing & Launch", color: MIDNIGHT, content: (<div><div style={{ fontSize: "13px", color: "var(--muted)", lineHeight: 1.7, marginBottom: "14px" }}>"Startups take off because founders make them take off." — Paul Graham. Landing page = 24/7 salesperson. Headline + Screenshot + CTA.</div><div style={{ display: "grid", gap: "6px" }}>{toolCard({ name: "Framer", desc: "Designer-quality landing page", url: "https://framer.com", free: false, tag: "$10/mo" }, MIDNIGHT)}{toolCard({ name: "Product Hunt", desc: "Global launch platform #1", url: "https://producthunt.com", free: true }, MIDNIGHT)}</div></div>) },
  ];
    ...
    <div style={{ marginBottom: "14px", display: "flex", flexDirection: "column", gap: "14px" }}>
      ...
      <StageWrapup
        ...
        doneItemsKo={[
          ...
          { label: "2. 빌드 방법 결정", detail: "AI 앱 빌더(Lovable·v0)·AI 코딩 어시스턴트(Cursor·Claude Code) 기반 프론트·백엔드 스택 확정" },
        ...
        verifyItemsKo={[
          ...
          "지재권 — 브랜드명·로고 상표권 사전 출원(선점 필수). 소스코드·콘텐츠 저작권은 창작 즉시 자동 발생(무방식주의 — 등록 불요, 증빙만 관리). 특허(핵심 알고리즘 — 신규성·진보성)·디자인권(화면 UI — 신규성·창작성)은 랜딩·데모·베타·보도 등 공개 '전'에 (가)출원 완료가 안전 — 공개 후 12개월 공지예외는 '본인 공개'만 구제하고, 제3자가 먼저 모방·출원하면 선출원주의로 무력화되어 권리를 영구 상실",
          "AI 생성 코드 — Cursor·Claude Code 등이 출력한 코드에 오픈소스(GPL 등 카피레프트) 라이선스 코드가 섞이면 소스 전면 공개 요구·저작권 소송 리스크 → 라이선스 저촉 스캔 필수 + 기업 소스 보안 위해 AI 도구 학습 데이터 제외(Opt-out)·비공개 모드 활성화",
