      ...
      {(() => {
        ...
        const modeContent: Record<string, {
          ...
          seed: {
            ...
            resources: [
              // 2026-05-12 확장: 한국 시드 단계 채용 채널 통합 (직군별·시니어용·커뮤니티)
              ...
              { name: ko ? "원티드 (스타트업 표준)" : "Wanted (startup standard)", desc: ko ? "IT 집중 + 추천 보상금. 시드~시리즈B 1순위" : "IT focused, referral bonus. Seed-B #1", url: "https://www.wanted.co.kr" },
              { name: ko ? "리멤버 커리어 (시니어·경력직)" : "Remember Career (senior)", desc: ko ? "MAU 506만, 명함 기반 시니어 헤드헌팅 1순위" : "5M MAU, business-card-based senior", url: "https://career.rememberapp.co.kr" },
              { name: ko ? "점핏 (개발자 전문)" : "Jumpit (dev-focused)", desc: ko ? "사람인 운영. 개발자 채용 + 연봉 인사이트" : "Saramin-run. Devs + salary insights", url: "https://jumpit.saramin.co.kr" },
              { name: ko ? "로켓펀치 (초기 스타트업)" : "RocketPunch", desc: ko ? "초기 스타트업 채용·투자자 매칭" : "Early-stage hires + investor match", url: "https://www.rocketpunch.com" },
              { name: ko ? "잡플래닛·크레딧잡 (연봉·평판 검증)" : "Jobplanet / Kredit Job", desc: ko ? "회사 평판 + 4대보험 기반 연봉 시세 검증" : "Reviews + insurance-based salary data", url: "https://www.jobplanet.co.kr" },
              { name: ko ? "OKKY / GeekNews / 디스콰이엇" : "OKKY / GeekNews / Disquiet", desc: ko ? "개발자 커뮤니티 채용 게시판 + GitHub-based 추천" : "Dev community boards + GH-based referrals", url: "https://okky.kr" },
      ...
      {false && (() => {
        ...
        const modeContent: Record<string, {
          ...
          indie: {
            ...
            tools: [
              { name: "홈택스 (개인사업자 등록)", desc: ko ? "5분 온라인 무료 등록" : "Free online registration", url: "https://hometax.go.kr/websquare/websquare.html?w2xPath=/ui/pp/index_pp.xml" },
