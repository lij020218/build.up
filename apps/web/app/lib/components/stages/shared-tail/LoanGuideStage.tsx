...
export function LoanGuideStage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "14px", marginBottom: "14px" }}>
      ...
      <LoanFaqCard
        ko={ko}
        qaText={d.knowledgeQaText}
        setQaText={d.setKnowledgeQaText}
        qaStatus={d.knowledgeQaStatus}
        qaError={d.knowledgeQaError}
...
// ─────────────────────────────────────────────────────────────────────────
//  LoanFaqCard — 대출·정책자금 질문 카드 (FAQ 우선 + AI fallback)
//
//  TaxFaqCard 와 동일 패턴. 검증된 FAQ 12개 + 매칭 0건 시만 AI fallback.
// ─────────────────────────────────────────────────────────────────────────

function LoanFaqCard({
  ko,
  qaText,
  setQaText,
  qaStatus,
  ...
  const matches: LoanFaqEntry[] = useMemo(() => {
    const q = search.trim();
    if (q.length < 2) return [];
    return matchLoanFaq(q);
  }, [search]);

  const displayedFaqs: LoanFaqEntry[] = matches.length > 0 ? matches : LOAN_FAQ_ENTRIES;
  ...
  return (
    <div style={{ marginTop: "18px", marginBottom: "18px" }}>
      <div style={sectionLabel}>{ko ? "자주 묻는 대출·정책자금 질문" : "Frequently Asked Loan Questions"}</div>
      <div style={cardStyle}>
        <div style={{ marginBottom: "12px" }}>
          <input
            ...
            placeholder={ko ? "예: 정책자금, 신용점수, 새출발기금, TIPS, 부결 사유..." : "e.g., policy fund, credit, TIPS..."}
            style={{
              width: "100%",
              padding: "10px 14px",
              borderRadius: "10px",
        ...
        {noMatch ? (
          <div style={{
            ...
            <div style={{ fontSize: "12px", color: "rgba(15,23,42,0.65)", lineHeight: 1.55, marginBottom: "12px" }}>
              {ko
                ? "AI 답변은 일반 가이드. 실제 신청 전 소진공 1357·1533-0100, 또는 거래 은행에 직접 확인 권장."
                : "AI answers are general guidance. Verify with SEMAS or your bank before applying."}
            </div>
            <button
              type="button"
              onClick={askAi}
