...
export function PreLaunchFinalStage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "14px", marginBottom: "14px" }}>
      ...
      <StageWrapup
        ko={ko}
        nextStageLabelKo="실전 운영 (대시보드)"
        doneItemsKo={[
          { label: isStartup ? "1. 출시 D-7 점검" : "1. 오픈 D-7 점검", detail: isStartup ? "프로덕션 배포·에러 추적·상태 페이지 작동 확인 + 가입→온보딩→결제 플로우 라이브 완주" : isOnline ? "스토어센터·상세페이지·가격·결제 라이브 확인 (실물은 재고·포장재 / 디지털은 전달·다운로드 방식)" : "POS·메뉴판·재고·인력·SNS·블로그 사전 모두 라이브 가능 상태 확인" },
          { label: isStartup ? "2. 런치 이벤트" : "2. 그랜드오픈 이벤트", detail: isStartup ? "런치 데이 — Product Hunt·커뮤니티·앱스토어 공지 + 첫 사용자 온보딩 지원" : isOnline ? "오픈 당일 첫 구매 혜택·알림받기 쿠폰 발행·인스타 리뷰 이벤트" : "오픈 데이 첫 손님 우대·시그니처 메뉴 시연·SNS 인증 이벤트" },
          { label: isStartup ? "3. 초기 레퍼런스 확보" : "3. 진성 리뷰 3개 확보", detail: isStartup ? "첫 사용자 후기·레퍼런스 3건 확보 (인터뷰·커뮤니티·케이스)" : isOnline ? "첫 주 네이버 쇼핑 상품 리뷰·인스타·블로그 진성 리뷰 3개 목표" : "첫 주 네이버 플레이스·인스타·블로그 진성 리뷰 3개 목표" },
          { label: "4. D+7 데이터 수집", detail: isStartup ? "가입·활성(WAU)·전환·리텐션 4개 지표 매일 모니터링" : isOnline ? "재구매율·평점·문의·주문량 4개 지표 매일 모니터링" : "재방문률·평점·문의·주문량 4개 지표 매일 모니터링" },
        ]}
        verifyItemsKo={isStartup ? [
          ...
          isOnline
            ? "온라인 — 상세페이지 필수 표시정보(사업자 정보·반품/교환 조건) + 실물은 KC 인증·재고, 디지털은 전달 방식·라이선스까지 최종 점검"
            : "음식점 — 식약처 「위생 등급제」 자율 신청 권장, 광고에 등급 표시 시 신뢰감 + 단속 제외",
        ]}
        nextSummaryKo="오픈·그랜드오픈·D+7 준비 완료 → 로드맵 전 과정 수료, 마이 대시보드 실전 운영(매출·재고·CS) 데이터 연동으로 진입"
      />
      )}
    </div>
  );
