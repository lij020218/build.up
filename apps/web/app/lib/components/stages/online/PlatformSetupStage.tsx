...
export function PlatformSetupStage() {
  ...
  const platforms: PlatItem[] = [
    { id: "smartstore", name: ko ? "네이버 스마트스토어" : "Naver Smartstore", desc: ko ? "쇼핑 검색 1위 · 결제수수료 최저" : "#1 shopping search · Lowest fee", color: "#03C75A", url: "https://sell.smartstore.naver.com", fee: ko ? "결제 1.98~3.63% + 유입 2% (2025.10 인하)" : "Payment 1.98~3.63% + Inflow 2% (2025.10 cut)", mau: "536만", pros: ko ? ["네이버 검색 노출 최강", "결제 수수료 최저 수준", "쇼핑라이브 가능", "스마트스토어 센터 무료"] : ["Best Naver search", "Lowest fees", "Shopping Live"], cons: ko ? ["광고 없이 초기 노출 어려움", "경쟁 셀러 매우 많음"] : ["Hard initial exposure", "Many competitors"] },
    { id: "coupang-mp", name: ko ? "쿠팡 마켓플레이스" : "Coupang Marketplace", desc: ko ? "이커머스 MAU 1위 · 로켓그로스" : "#1 ecommerce MAU · Rocket Growth", color: "#1460F3", url: "https://wing.coupang.com", fee: ko ? "4~10.8% + 월매출 100만원↑ 시 55,000원" : "4~10.8% + ₩55K/mo (only when sales ≥₩1M)", mau: "3,339만", pros: ko ? ["최대 트래픽 (MAU 3,339만)", "로켓그로스 풀필먼트", "와우 멤버십 노출 우선"] : ["Most traffic", "Rocket Growth", "Wow priority"], cons: ko ? ["월매출 100만원 이상 시 정액 55,000원 (그 이하 X)", "가격 경쟁 심화", "수수료 높은 편"] : ["₩55K monthly (when sales ≥₩1M)", "Price competition", "Higher fees"] },
