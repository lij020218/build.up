...
export function StageGuideViewer() {
          ...
          return (
            <div key={si} style={{ marginBottom: "10px" }}>
              ...
              <div style={{ background: "white", borderRadius: "20px", overflow: "hidden", boxShadow: "0 2px 16px rgba(0,0,0,0.07), 0 1px 4px rgba(0,0,0,0.04)" }}>
                {supply.items.map((item, ii) => {
                  ...
                  return (
                    <div key={ii}>
                      ...
                      >
                        {/* select indicator */}
                        <div style={{ flexShrink: 0, width: "20px", height: "20px", borderRadius: "50%", border: isSelected ? "none" : "1.5px solid rgba(0,0,0,0.18)", background: isSelected ? "rgb(59,92,140)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s" }}>
                          {isSelected && (
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "2px", flexWrap: "wrap" as const }}>
                            <span style={{ fontSize: "14px", fontWeight: isSelected ? 640 : 590, color: isSelected ? "rgb(59,92,140)" : "var(--text)", letterSpacing: "-0.3px" }}>{item.name}</span>
                ...
                >
                  <div style={{ flexShrink: 0, width: "20px", height: "20px", borderRadius: "50%", border: selectedName === etcKey ? "none" : "1.5px solid rgba(0,0,0,0.18)", background: selectedName === etcKey ? "rgb(59,92,140)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s" }}>
                    {selectedName === etcKey && (
                      <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                        <path d="M2 5L4.2 7.5L8 3" stroke="white" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: "14px", fontWeight: selectedName === etcKey ? 640 : 500, color: selectedName === etcKey ? "rgb(59,92,140)" : "rgba(0,0,0,0.5)", letterSpacing: "-0.2px" }}>
  ...
  const operationsEl: React.ReactNode = (currentStage.code as string) !== "operations_setup" || guideStepIndex === 0 ? null : (() => {
    ...
    const logisticsItems: OpsItem[] = businessCtx.isDeliveryRelevant
      ? [
          ...
          { id: "ddangyo",    name: ko ? "땡겨요" : "Ddangyo",         desc: ko ? "MAU 345만 · 공공배달앱 1위" : "MAU 3.45M · #1 public app",       color: "#b64c4c", url: "https://www.ddangyo.com",     fee: ko ? "2% 고정" : "2% fixed",                     mau: "345만",   pros: ko ? ["수수료 최저", "공공앱 신뢰감", "서울·경기 강세"] : ["Lowest fee", "Public trust"], cons: ko ? ["전국 커버리지 부족", "유입 제한적"] : ["Limited nationwide", "Lower traffic"] },
      ...
      : isDigitalCategory
      ? [
          { id: "smartstore", name: ko ? "네이버 스마트스토어" : "Naver Smartstore", desc: ko ? "MAU 536만 · 쇼핑 검색 1위" : "MAU 5.36M · #1 shopping search", color: "#03C75A", url: "https://sell.smartstore.naver.com", fee: ko ? "결제 1.98~3.63% + 유입 2% (2025.10 인하)" : "Payment 1.98~3.63% + Inflow 2% (2025.10 cut)", mau: "536만", pros: ko ? ["네이버 검색 노출 최강", "결제 수수료 최저", "쇼핑라이브 가능"] : ["Best Naver search exposure", "Lowest payment fee"], cons: ko ? ["광고 없이 노출 어려움", "경쟁 치열"] : ["Hard to get exposure without ads"] },
          { id: "coupang-mp", name: ko ? "쿠팡 마켓플레이스" : "Coupang Marketplace", desc: ko ? "MAU 3,339만 · 이커머스 1위" : "MAU 33.4M · #1 ecommerce", color: "#E52222", url: "https://wing.coupang.com", fee: ko ? "4~10.8% + 월매출 100만원↑ 시 55,000원" : "4~10.8% + \u20A955K/mo (only when sales ≥\u20A91M)", mau: "3,339만", pros: ko ? ["최대 트래픽", "로켓그로스 풀필먼트", "와우 멤버십 노출"] : ["Most traffic", "Rocket Growth fulfillment"], cons: ko ? ["월매출 100만원↑ 시에만 월 정액 55,000원", "가격 경쟁 심화"] : ["\u20A955K monthly (when sales ≥\u20A91M)", "Price competition"] },
    ...
    const snsChannels: OpsItem[] = [
      ...
      { id: "google-business",name: "구글 비즈니스",       desc: ko ? "구글맵 노출 · 외국인 필수" : "Google Maps · Essential for foreigners",           color: "#4285F4", url: "https://business.google.com/kr/business-profile/", fee: ko ? "무료" : "Free", pros: ko ? ["완전 무료", "구글맵 노출", "외국인 접근성"] : ["Free", "Google Maps", "Foreign customers"], cons: ko ? ["한국 내 검색 점유율 낮음"] : ["Low domestic search share"] },
    ];
  ...
  const registrationSetupEl: React.ReactNode = (() => {
    ...
    const step1: RegStepData = {
      ...
      links: ko ? [
        { text: "\uad6d\uc138\uccad \ud648\ud0dd\uc2a4 \u2014 \uc0ac\uc5c5\uc790\ub4f1\ub85d \uc2e0\uccad", href: "https://hometax.go.kr/websquare/websquare.html?w2xPath=/ui/pp/index_pp.xml&menuCd=index3" },
        { text: "KIPRIS \u2014 \uc0c1\ud45c\xb7\uc0c1\ud638 \uac80\uc0c9", href: "https://www.kipris.or.kr" },
      ] : [
        { text: "Hometax \u2014 Business registration", href: "https://hometax.go.kr/websquare/websquare.html?w2xPath=/ui/pp/index_pp.xml&menuCd=index3" },
    ...
    const step4: RegStepData = {
      ...
      links: ko ? [
        ...
        { text: "\ud648\ud0dd\uc2a4 \u2014 \ud604\uae08\uc601\uc218\uc99d \uac00\ub9f9 \uc2e0\uccad", href: "https://hometax.go.kr/websquare/websquare.html?w2xPath=/ui/pp/index_pp.xml&menuCd=index3" },
        ...
        { text: "Hometax \u2014 cash receipt merchant", href: "https://hometax.go.kr/websquare/websquare.html?w2xPath=/ui/pp/index_pp.xml&menuCd=index3" },
      ...
      <div style={{ display: "grid", gap: "14px", marginTop: "2px" }}>
        ...
        {step.traps.length > 0 && (
          <div style={{ display: "grid", gap: "6px" }}>
            {step.traps.map((trap) => (
              <div key={trap.label} style={{ display: "grid", gap: "3px", padding: "11px 13px", borderRadius: "13px", background: "rgba(182,76,76,0.05)", border: "1px solid rgba(182,76,76,0.13)" }}>
                <div style={{ fontSize: "12px", fontWeight: 640, color: "#b64c4c" }}>\u26a0 {trap.label}</div>
