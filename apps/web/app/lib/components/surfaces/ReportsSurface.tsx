...
const finLabel: React.CSSProperties = {
  fontSize: 12, fontWeight: 750, color: "#191970", letterSpacing: "0.04em", textTransform: "uppercase",
};
...
export function ReportsSurface() {
  ...
  const c = useDashboardComputed(d); // 재무 전망 섹션용 (2026-07-24)
  ...
  return (
    <main style={pageStyle}>
      <header style={headerStyle}>
        ...
        <h1 style={titleStyle}>{ko ? "재무" : "Finance"}</h1>
        <p style={subtitleStyle}>
          {ko
            ? "이번 달 손익분기, 13주 자금흐름, 12개월 시뮬레이션 — 그리고 일·주·월 보고서까지. 모든 수치는 사장님 입력 데이터 기반."
            : "Break-even, 13-week cash flow, 12-month simulation — plus your daily/weekly reports. Grounded in your inputs only."}
        </p>
      </header>

      {/* ━━━ 재무 전망 (2026-07-24 신설) — 이번 달 → 13주 → 12개월, 대시보드에서 이관 ━━━ */}
      <section style={{ display: "grid", gap: 14, marginBottom: 28 }}>
        {/* ① 이번 달 손익분기 트래커 (월간 진행 카드 흡수) */}
        {(() => {
          ...
          return (
            <div style={finCard}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 6 }}>
                <div style={finLabel}>{ko ? "이번 달 손익분기" : "This month vs break-even"}</div>
                <span style={{ fontSize: 13, fontWeight: 750, color: over ? "#1d3557" : "#191970" }}>
                  {pct}% {over ? (ko ? "· 손익분기 달성" : "· reached") : ""}
                </span>
              </div>
              <div style={{ height: 8, borderRadius: 4, background: "rgba(25,25,112,0.08)", margin: "10px 0 8px" }}>
                <div style={{ width: `${pct}%`, height: "100%", borderRadius: 4, background: over ? "#1d3557" : "#191970", transition: "width .3s" }} />
              </div>
        ...
        {/* ② 생존 보드 + 13주 자금흐름 (대시보드 Tier2 에서 이관) */}
        <SurvivalBoardCard
          ko={ko}
        ...
        {/* ③ 12개월 시뮬레이션 — what-if 레버 + 성장 시나리오 (Tier4 What-If 흡수·확장) */}
      ...
      <div style={{ fontSize: 13, fontWeight: 750, color: "rgba(15,23,42,0.55)", margin: "0 0 10px" }}>
        {ko ? "보고서" : "Reports"}
      </div>
