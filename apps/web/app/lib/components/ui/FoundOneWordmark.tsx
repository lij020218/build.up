// Found.One 워드마크 — 공식 로고 서체(테크노 지오메트릭). 사장님 제공 SVG(래스터 자동추적)
//   기반이되, 추적이 깨뜨린 "F" 글자(중간 가로바 누락)를 정식 F 로 보정(2026-07-14).
//   · 글자 fill = currentColor → 부모 색 상속(다크 랜딩=흰색 / 라이트=검정 자동 대응).
//   · "." 은 마크색(미드나잇 네이비) 액센트.
//   · viewBox 는 글자+점 타이트 바운드(원본 757×138 중 워드마크 영역).
import type { CSSProperties } from "react";

// F (보정): 상단바 + 중간바 + 좌측 스템을 이어붙인 정식 F 글리프.
const LETTER_F = "M 177 54 L 227 54 L 227 60 L 183 60 L 183 72 L 217 72 L 217 78 L 183 78 L 183 98 L 177 98 Z";
// O U N D O N E (원본 추적 그대로 — F 만 위에서 교체).
const LETTERS_REST =
...
const DOT = "M 532 88 L 529 91 L 529 97 L 531 99 L 537 99 L 539 97 L 539 96 L 540 95 L 540 92 L 539 91 L 539 90 L 538 89 L 537 89 L 536 88 Z";
...
export function FoundOneWordmark({
  ...
  return (
    ...
    >
      <path d={`${LETTER_F} ${LETTERS_REST}`} fill={color} fillRule="evenodd" />
