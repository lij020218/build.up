// Found.One 워드마크 — 공식 로고 서체(테크노 지오메트릭). 사장님 제공 SVG의 글자 패스를
//   **원본 그대로** SSOT로 사용(2026-07-14 정정). 이전엔 F를 솔리드로 잘못 교체하고 두 번째
//   O의 안쪽 카운터를 누락했었음 → 원본 path 전체 복원.
//   · F 는 상단바가 살짝 떨어진 테크노 스타일(좌측 스템 상단 빈틈)이 디자인의 특징.
//   · O·D 는 외곽+안쪽 카운터 2서브패스 → fillRule="evenodd" 로 속이 뚫림.
//   · 글자 fill = currentColor(다크 배경=흰색 / 라이트=검정 자동). "." 은 마크색 액센트.
import type { CSSProperties } from "react";

// 원본 SVG path-2 전체(글자 F O U N D O N E). 각 O·D 는 외곽+카운터 2서브패스 → evenodd 필수.
const LETTERS =
...
// 타이트 바운드: 글자 x 176~750 · y 54~98, 점 y~99. 약간 패딩.
const VB_X = 172, VB_Y = 50, VB_W = 582, VB_H = 54;
const ASPECT = VB_W / VB_H;

export function FoundOneWordmark({
  height = 20,
  color = "currentColor",
  dotColor = "#172A78",
  style,
  ...
  return (
    ...
    >
      <path d={LETTERS} fill={color} fillRule="evenodd" />
      <path d={DOT} fill={dotColor} fillRule="evenodd" />
    </svg>
  );
}

export default FoundOneWordmark;
