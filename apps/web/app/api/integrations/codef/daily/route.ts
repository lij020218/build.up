...
import { getCodefCardDailyEntries } from "../../../_lib/codef-revenue-bridge";
...
export async function GET(request: Request) {
  ...
  const rl = await checkSimpleRateLimit({ key: `codef-daily:${auth.userId}`, limit: 60, windowMs: 60_000 });
  ...
  try {
    const entries = await getCodefCardDailyEntries(supabase, auth.userId, { from });
    return NextResponse.json(
      { ok: true, entries, range: { fromDays: safeDays } },
      { headers: { "Cache-Control": "private, max-age=60, stale-while-revalidate=300" } }
    );
  } catch (e) {
    console.warn("[codef/daily] graceful empty:", (e as Error).message);
