/**
 * GET /api/integrations/stripe/status — 사장님 Stripe 연결 상태 + 최근 30일 이벤트 요약.
 * 평문 Secret 절대 반환 X (mask 만).
 */
...
export async function GET(request: Request) {
  ...
  const { data: conn } = await supabase
    ...
    .select("secret_mask, account_id, status, encrypted_webhook_secret, last_validated_at, last_sync_at, last_sync_error, created_at")
  ...
  return NextResponse.json({
    ...
    status: conn.status,
    hasWebhookSecret: !!conn.encrypted_webhook_secret,
