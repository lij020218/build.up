...
import { fetchIndustryStartupCosts } from "@build-up/shared";

export async function GET(request: Request) {
  const route = "/api/data/franchise/industry-costs";
  const requestId = getRequestId(request);
  const auth = await requireApiUser(request);
  if (!auth.ok) return NextResponse.json({ error: auth.error }, { status: auth.status });

  const rl = await checkSimpleRateLimit({ key: `data-industry-cost:${auth.userId}`, limit: 20, windowMs: 60_000 });
  ...
  try {
    const result = await fetchIndustryStartupCosts(
      { apiKey, baseUrl: "" },
      {
    ...
    return NextResponse.json({ error: "업종별 창업비용 조회 실패", detail: error instanceof Error ? error.message : "" }, { status: 502 });
  }
}
