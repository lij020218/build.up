...
import { fetchDisclosureBody } from "@build-up/shared";

export async function GET(request: Request) {
  const route = "/api/data/franchise/disclosure";
  const requestId = getRequestId(request);
  const auth = await requireApiUser(request);
  if (!auth.ok) return NextResponse.json({ error: auth.error }, { status: auth.status });

  const rl = await checkSimpleRateLimit({ key: `data-disclosure:${auth.userId}`, limit: 20, windowMs: 60_000 });
  ...
  try {
    const result = await fetchDisclosureBody(
      { apiKey, baseUrl: "" },
      {
        ...
        numOfRows: url.searchParams.get("numOfRows") ? Number(url.searchParams.get("numOfRows")) : 10,
      }
    );
    ...
    return NextResponse.json({ error: "정보공개서 조회 실패", detail: error instanceof Error ? error.message : "" }, { status: 502 });
  }
}
