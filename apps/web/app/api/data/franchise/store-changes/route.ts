...
export async function GET(request: Request) {
  ...
  const rl = await checkSimpleRateLimit({ key: `data-store-changes:${auth.userId}`, limit: 20, windowMs: 60_000 });
  ...
  try {
    const result = await fetchStoreChanges(
      { apiKey, baseUrl: "" },
      {
    ...
    return NextResponse.json({ error: "가맹점 증감 조회 실패", detail: error instanceof Error ? error.message : "" }, { status: 502 });
  }
}
