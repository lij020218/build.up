...
import { PRODUCTS_PARSE_RESPONSE_SCHEMA } from "./schema";
...
- 데이터를 파싱할 수 없거나 제품 정보가 없으면 {"products":[]} 을 반환하세요.
...
- 데이터가 카페/음식 제품이 아니어도 그대로 추출하세요 (양말, 의류, 전자제품 등 모든 종류).`
        : `Parse product data from Excel/CSV/text into JSON. Respond ONLY with JSON.
...
- If data cannot be parsed or contains no products, return {"products":[]}.
...
- Skip header/total rows. Extract ALL product types (clothing, electronics, food, etc).`,
      ...
      response_schema: PRODUCTS_PARSE_RESPONSE_SCHEMA, // Structured Outputs — 파서는 안전망
    });
