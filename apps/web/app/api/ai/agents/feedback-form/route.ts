...
export async function POST(request: Request) {
  ...
  return runAiFeature({ request, feature: "agents-feedback-form" }, async () => {
    ...
    // 실패 시 throw → 게이트가 1회 재시도 후 환불 + 503 (잘린 JSON 은 parseLlmJson 이 복구 시도)
    const parsed = parseLlmJson<{
      intro?: unknown; questions?: unknown; tips?: unknown; paperText?: unknown;
    }>(text);
