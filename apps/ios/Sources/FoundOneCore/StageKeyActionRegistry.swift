...
public enum BUStageKeyActionRegistry {
    ...
    private static let actions: [String: BUStageKeyAction] = [
        ...
        "industry-selection": .init(
            ...
            detail: "한 업종을 고르면 입지·메뉴·인허가·세무까지 단계별 가이드가 자동 맞춤. 멀티 카테고리 분산보다 한 업종 집중이 첫 12개월 매출에 4배 효과."
        ),
        "startup-type": .init(
            title: "창업 형태가 모든 절차·세무·자금원을 결정합니다",
            detail: "프랜차이즈 · 자영업 · 스타트업 — 형태별로 인허가·자금·세무 경로가 완전히 다름. 한 번 선택하면 이후 로드맵이 그 형태에 맞게 분기."
        ),
        "business-model": .init(
            title: "한 가지 운영 모델에 우선 락인 — 분산은 매출 후",
            detail: "먼저 한 가지 운영 모델로 락인하고 첫 3개월은 그 모델로 표준화하세요. 매출이 안정된 뒤 두 번째 채널·모델을 추가하는 게 실패 위험을 줄입니다 — 여러 채널을 동시에 벌이면 초기 운영·마케팅 역량이 분산됩니다."
        ),
        ...
        "location-candidates": .init(
            title: "최종 1곳을 정하기 전 — 후보 3곳 이상 동일 기준 점수화",
            detail: "상권 선택은 1~2년 묶이는 의사결정. 임대료·유동인구·경쟁 밀도·타겟 적합도를 동일 기준으로 비교해야 후회 없음."
        ),
        "financial-review": .init(
            title: "8개 비용 항목 확인 → Day 1부터 운영 대시보드 정확 가동",
            detail: "이전 단계 임대료·인건비·수수료 자동 집계. 업종 평균과 비교해 검증."
        ),
        "menu-design": .init(
        ...
        "permit-check": .init(
            ...
            detail: "임대 계약 후 「영업 불가」 발견 시 보증금 1,000~5,000만원 즉시 묶임. 지금은 발급이 아닌 “무엇이 필요한지” 만 파악하는 사전 매핑 단계입니다."
        ),
        "contract-review": .init(
            title: "계약서 사인 전 75분 — 보증금 1,000~5,000만원을 지키는 시간",
            detail: "건물·계약·보호 3축을 점검하고 5종 특약을 협상해야 합니다. 사인 후 발견하는 문제는 거의 100% 임차인 부담."
        ),
        "construction-setup": .init(
            title: "보건증은 공사 중 발급 + 다중이용업(100㎡↑)이면 착공 전 소방 '안전시설 설치신고' — 완비증명(소방필증)은 완공검사 후라 늦으면 오픈 지연",
            detail: "인테리어 컨셉 → 견적 2곳 비교 → 설계 확정 → 시공 → 완공검사. 다중이용업(바닥 100㎡↑·지하 66㎡↑)이면 소방 '안전시설등 설치신고'는 착공 전(처리 3일), '완비증명(소방필증)'은 완공신고·현장확인 후 발급(3일) — 그 외 업종은 소화기·비상구. 보건증은 공사 중 미리 발급. 설치신고를 공사 끝나고 하면 그만큼 오픈이 밀린다."
        ),
        "vendor-setup": .init(
            title: "공급처·장비를 지금 발주하세요",
            detail: "개업 4–6주 전이 구매·계약의 골든타임입니다. 한국에서 검증된 도매처와 장비를 견적 → 비교 → 선납 순서로 진행하세요."
        ),
        ...
        "insurance-tax-setup": .init(
            ...
            detail: "직원 1명만 채용해도 4가지 의무 자동 발생: ① 근로계약서 (미체결 500만원 이하 벌금·과태료) + ② 4대보험 취득신고(건강14일·기타 익월15일) + ③ 원천세 매월 10일 + ④ 급여 시스템. 5인 미만 예외 X. 두루누리 80% 지원 가능."
        ),
        "hiring-setup": .init(
            title: "첫 직원 채용 60분 — 근로계약서 + 4대보험 + 원천세 3축 점검",
            detail: "공고 → 면접 → 계약서 2부 + 1부 교부 → 4대보험 취득 신고(건강14일·기타 익월15일) → 매월 원천세. 빠뜨리면 과태료 + 임금 체불 신고 + 세무조사 리스크."
        ),
        ...
        "pre-launch": .init(
            title: "소프트 오픈 — 본 오픈 첫 달 매출을 결정하는 리허설",
            detail: "지인·동네 주민·인플루언서 초대 → 실 운영 1회전 → 피드백 → 본 오픈 직전 보강. 사인된 직원·POS·SNS 가 처음으로 함께 돌아가는 통합 테스트입니다."
        ),
