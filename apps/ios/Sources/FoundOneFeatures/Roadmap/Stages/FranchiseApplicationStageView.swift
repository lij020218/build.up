...
private let contractCheckpoints: [ContractCheckpoint] = [
    ...
    .init(id: "fee-refund",         title: "가맹비 반환 조건",
          description: "계약 해지 시 가맹비 반환 여부와 조건을 확인하세요. 정보공개서 미제공·허위정보 제공 시 가맹금 반환을 청구할 수 있으나, 반드시 계약 체결일로부터 4개월 이내에 서면으로 요구해야 효력이 있습니다 (가맹사업법 §10 · 무조건 전액은 아님).",
          riskLevel: "critical"),
    ...
    private var whyPage: some View {
        VStack(alignment: .leading, spacing: BUSpacing.cardGap) {
            ...
            BUCard(.card) {
                VStack(alignment: .leading, spacing: 10) {
                    BUEyebrow("한국 가맹 데이터")
                    factRow(tag: "대표 분쟁 유형", body: "인테리어 강제·해지 위약금은 대표적 가맹 불공정거래 유형 — 지정업체 강요는 '거래상대방 구속'으로 공정위 시정조치 대상")
