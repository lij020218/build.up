//
//  RegistrationSetupStageView.swift — 사업자등록 + 인허가 (iOS 네이티브)
//
//  2026-06-25 SSOT 전환:
//    콘텐츠를 @foundone/shared 의 registration-setup SSOT(JSON codegen)로 이전.
//    iOS는 BUStageContentRenderer, 웹은 StageContentRenderer 가 같은 데이터를 렌더 →
//    web↔iOS 무드리프트(제목·내용 차이)가 구조적으로 불가능.
//
//    콘텐츠 수정은 packages/shared/src/stages/content/registration-setup.ts 한 곳에서만 하고
//    `npx tsx scripts/gen-stage-content-json.mts` 로 stage-content.json 재생성.
//
//  웹 SSOT: apps/web/app/lib/components/stages/offline/RegistrationSetupStage.tsx
//  stageId: "registration-setup"
//

import SwiftUI
import FoundOneComponents
import FoundOneCore

public struct RegistrationSetupStageView: View {
    private let stageId = "registration-setup"

    public init() {}

    public var body: some View {
        if let content = StageContentRegistry.content(for: stageId) {
            BUStageContentRenderer(content: content)
        } else {
            // 콘텐츠 번들 누락 폴백 — 정상 빌드에선 도달하지 않음.
            ContentUnavailableView(
                "콘텐츠를 불러오지 못했어요",
                systemImage: "exclamationmark.triangle",
                description: Text("stage-content.json 번들을 확인하세요.")
            )
        }
    }
}

#if DEBUG
import FoundOneData

#Preview("RegistrationSetup") {
    let store = RoadmapStore()
    store.pathProvider = { _ in ["registration-setup"] }
    return RegistrationSetupStageView().environment(store)
}
#endif
