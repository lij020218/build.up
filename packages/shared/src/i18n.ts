...
> = {
  ...
  "cafe-delivery-hybrid": { ko: { title: "하이브리드 (매장+배달)", summary: "매장 운영 + 배달앱·픽업 병행 — 음료·디저트 배달 수요까지 함께 흡수합니다.", reasons: ["좌석 매출 + 배달·픽업 신규 수요 동시 확보", "디저트·음료 배달 수요가 빠르게 성장"], warnings: ["배민·쿠팡이츠 수수료·포장비가 원가에 반영됨", "음료·디저트 품질이 배달 중에도 유지돼야 함"], categoryLabel: "운영 방식" } },
...
const stageCopy: Record<string, { ko: { title: string; goal: string; whyNow: string } }> = {
  ...
  insurance_tax_setup: { ko: { title: "직원 채용 의무 — 근로계약·4대보험·급여 세팅", goal: "첫 직원 채용 시 즉시 발생하는 4가지 의무: ① 근로계약서 작성 (미체결 시 500만원 이하 벌금·과태료) ② 4대보험 가입 (건강 14일·기타 익월 15일) ③ 원천세 매월 신고 ④ 급여 시스템 결정 (수기·세무사·SaaS).", whyNow: "직원 1명만 채용해도 4대보험 취득 신고 의무 (건강 14일·기타 익월 15일). 미신고 = 가산세 + 소급납부. 두루누리 80% 지원 (270만 미만, 10인 미만, 36개월) 신고 시점에만 신청 가능 — 놓치면 비용 폭증." } },
...
const taskTitleCopy: Record<string, { ko: string }> = {
  ...
  "md-pricing-set": { ko: "각 항목 가격 책정 — 원가율 목표 설정 (외식업 기준 33% 이하)" },
  ...
  "music-license-registered": { ko: "매장 배경음악 공연권료 — 카페·주점·헬스장 등 지정 업종 50㎡(15평) 이상만 의무(일반 식당·미용실·편의점 면제), 월 4천원~. KOMCA 또는 매장음악서비스 가입" },
  ...
  "hygiene-health-checked": { ko: "위생교육·건강진단(보건증) 요건 확인 — 내 업종 기준 교육시간·발급처 (위생교육은 영업신고하는 대표자만 이수, 직원은 보건증)" },
  "license-registration-checked": { ko: "전문 면허·업종별 등록 요건 확인 — 내 업종에 해당하는 면허·등록만 (해당 없으면 스킵)" },
  ...
  // construction-setup
  "interior-concept-selected": { ko: "디자인 컨셉 방향 선택 — 아래 업종별 컨셉 후보 중 1안 결정" },
  ...
  "fire-health-parallel": { ko: "보건증은 공사 중 사전 발급 + 다중이용업(100㎡↑)이면 소방 '안전시설 설치신고'는 착공 전·완비증명(소방필증)은 완공검사 후 (그 외 업종은 소화기·비상구)" },
  // vendor-setup
