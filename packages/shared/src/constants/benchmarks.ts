...
export const LEGAL = {
  ...
  /** 간이과세 기준 연매출 (원) — 2024.1.1 시행, 부가가치세법 시행령 §109 (간이→일반 자동 전환 기준) */
  SIMPLIFIED_TAX_THRESHOLD: 104_000_000,
  /** 4대보험 사업자 부담률 (%) — 2026 기준 (= hiring-cost.ts TOTAL_EMPLOYER_RATE_PCT 와 동일)
   *   국민연금 4.75 + 건강보험 3.595 + 장기요양 0.4724(=3.595×0.1314)
   *   + 고용보험(실업급여 0.9 + 고용안정·직업능력개발 0.25) + 산재 0.7(일반서비스업)
   *   ⚠️ 2026-06-08 정정: 종전 10.42 는 고용안정·직업능력개발 0.25% 누락(거짓값). 0.25% 포함이 법적 정답. */
  EMPLOYER_INSURANCE_RATE: 4.75 + 3.595 + 0.4724 + (0.9 + 0.25) + 0.7,
  /** 데이터 기준 연도 */
  DATA_YEAR: "2026",
} as const;
