...
const BUILDING_SUB: Record<string, string> = {
  ...
  beauty: "건축물대장 — 용도·소음·위반 표시를 확인합니다.",
  fitness: "건축물대장 — 용도·소음·위반 표시를 확인합니다.",
  education: "건축물대장 — 용도·소음·위반 표시를 확인합니다.",
  pet: "건축물대장 — 용도·소음·위반 표시를 확인합니다.",
  "living-service": "건축물대장 — 용도·소음·위반 표시를 확인합니다.",
  space: "건축물대장 — 용도·소음·위반 표시를 확인합니다.",
...
const FAVORABLE: Record<string, Favorable> = {
  ...
  "online-digital": { context: "온라인·디지털", recommendation: "부족 시설은 「임대인 1/2 부담 + 원상복구 면제」 특약으로", rationale: "사인 전 특약 명시. 보강 불가하거나 임대인이 거부하면 다른 매물이 낫습니다." },
  "startup-tech": { context: "스타트업", recommendation: "부족 시설은 「임대인 1/2 부담 + 원상복구 면제」 특약으로", rationale: "사인 전 특약 명시. 보강 불가하거나 임대인이 거부하면 다른 매물이 낫습니다." },
};

const BLD_WATCH_BASE: TrapItem = { label: "위반건축물 표시 = 영업신고 영구 불가", text: "건축물대장에 「위반건축물」 표기 시 무조건 매물 변경. 임대료 30% 싸도 입주 절대 불가." };
...
const FAC_WATCH_OTHER: TrapItem[] = [
  { label: "소방완비증명서 없이 영업 = 단속·과태료", text: "2층 이상 또는 100㎡↑ 또는 다중이용시설은 소방완비 의무. 임대 전 관할 소방서에 발급 가능 여부 확인." },
];
  ...
  byCategory[cat] = {
    ...
    workSteps: {
      ...
      facility: {
        ...
        watchouts: isFoodLike ? FAC_WATCH_FOOD : FAC_WATCH_OTHER,
      },
    },
  };
}
...
export const PERMIT_CHECK_CONTENT: StageContent = {
  ...
  pages: [
    ...
    },
    {
      ...
      label: "1. 건물",
      sections: [
        { kind: "workStep", axis: "building", stepLabel: "1. 건물 적합성", time: "10분", headline: "건축물대장으로 「용도 + 정화조 + 위반 표시」 확인" },
      ],
    },
    {
      id: "person",
      label: "2. 사람",
      sections: [
        { kind: "workStep", axis: "person", stepLabel: "2. 사람 적합성", time: "10분", headline: "면허·보건증·위생교육 — 영업 시작 전 의무" },
      ],
    },
    {
      id: "facility",
      label: "3. 시설",
      sections: [
        { kind: "workStep", axis: "facility", stepLabel: "3. 시설 적합성", time: "10분", headline: "소방완비증명서 + 환기·후드 + 가스·전기 안전 확인" },
      ],
    },
    {
      id: "negotiate",
      ...
      sections: [
        {
          ...
          tasks: [
            ...
            { id: "neg-2", title: "보강 가능 항목 = 비용 분담 특약", detail: "「임대인이 1/2 부담 + 임차 종료 시 원상복구 면제」 사인 전 특약. 평균 500만원 절약." },
  ...
  wrapup: {
    ...
    doneItems: [
      { label: "1. 건물 적합성", detail: "건축물대장 용도·정화조·위반 표시 — 영업 가능 건물인지 확정" },
      { label: "2. 사람 적합성", detail: "본인 면허·위생교육·보건증 일정 확정" },
      { label: "3. 시설 적합성", detail: "소방완비·환기·전기·가스 보강 가능 여부 확인" },
      { label: "4. 임대인 협상", detail: "빠진 항목 = 임대료 인하·보강 비용 분담 협상 카드로 정리" },
    ],
    verifyItems: [
      "건축물대장은 임대인 말이 아니라 정부24(gov.kr)에서 직접 발급해 확인 — 「위반건축물」 표시 1개면 영업신고 영구 불가",
      "정화조·용도지역·소음·배수 기준 — 업종별 영업 가능 여부를 계약 전에 확정 (음식점은 정화조 BOD 30평 7~10인용↑, 부족 시 증축 1,000만원+)",
      "면허·위생교육·보건증은 영업 시작 전 의무 — 사전 일정 확보로 오픈 지연 0",
      "소방완비증명서(2층 이상 또는 100㎡↑) 발급 가능 여부 — 임대 전 관할 소방서 확인",
      "환기·전기 용량 보강 가능 여부 — 임대 전 확인 (음식점은 외부 덕트 불가 매물이면 배달전문도 조리 불가)",
