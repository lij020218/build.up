...
const TRAP_SETUP_SMB: TrapItem[] = [
  { label: "현금영수증 — 의무발행업종은 미발급 시 20% 가산세 (내 업종 확인)", text: "의무발행업종(학원·전문직·병원·유흥주점·출장음식·소매 등)은 고객 요청 없어도 건당 10만원+ 의무 발급 → 미발급 20% 가산세. 일반음식점·미용실 등 소비자상대업종은 요청 시 발급(무조건 의무 아님). 내 업종이 의무발행인지 홈택스에서 확인." },
...
const TRAP_SAVINGS: TrapItem[] = [
  { label: "감가상각은 여러 해 분산이 유리 — 첫 해 전액 처리는 손해", text: "취득가 100만원 초과 자산은 내용연수(보통 5~10년)에 걸쳐 감가상각(100만원 이하는 그해 즉시 비용처리 가능). 첫 해에 다 털면 상각한도 초과분은 그해 비용 인정 안 됨(다음 해로 이월) + 매출 오르는 2~5년차에 쓸 비용이 없어 절세에 불리 — 여러 해 분산이 유리. 세무사·SaaS 자동 계산 활용." },
  { label: "복리후생비 ≠ 기업업무추진비(구 접대비) — 명확히 구분", text: "직원 대상 지출=복리후생비(전액), 거래처 식사=기업업무추진비(구 접대비, 한도). 단 대표 1인 단독 사업장의 대표 본인 식대는 복리후생비 처리 불가(필요경비 부인). 잘못 분류하면 비용 부인. ※2024년부터 '접대비'→'기업업무추진비'로 명칭 변경." },
];
...
const SMB_CHECKLIST = [
  ...
  { id: "tc-cash", label: "현금영수증 가맹점 등록", detail: "가맹점 등록 후 소비자 요청 시 발급 (음식점·미용 등은 소비자상대업종). 단 의무발행업종(학원·전문직·병원·유흥주점·출장음식 등)은 10만원+ 무조건 발급·미발급 시 20% 가산세 — 내 업종 확인", required: true },
...
export const TAX_GUIDE_CONTENT: StageContent = {
  ...
  wrapup: {
    ...
    doneItems: [
      { label: "1. 부가세 신고 일정", detail: "1·7월 확정신고(개인 일반) + 4·10월 예정고지 납부 · 간이과세는 1월 연 1회 (법인 창업만 분기 신고) + 자동이체 셋업" },
