...
유동인구·경쟁·임대료 수치를 **절대 생성하지 마세요** — 서버가 한국부동산원 실측 데이터로 이 필드를
통째로 대체합니다. 여기서 만든 수치는 사용자에게 전달되지 않으며, 만드는 것 자체가 규칙 위반입니다.

## ⚠️ 수치 그라운딩 (절대 규칙 — 환각 방지)
- 인허가 비용·기간, 정책자금/지원금 금액·자격, 세율·세액공제, 4대보험 요율은 **입력으로 주입된 구조화 데이터(legal.permitsDetailed, fundingPrograms.amount 등)와 아래 명시된 벤치마크 범위 안에서만** 답하라.
- 주입 데이터·아래 범위에 없는 구체 금액/기간/요율/지원사업은 **임의로 만들지 말고** "관할 기관(시·군·구청, 소상공인시장진흥공단, 국세청)에서 최신 확인 필요"로 표기하라.
- 특히 정책자금 한도·금리, 청년창업 세액감면율, 간이과세 기준금액 같은 매년 바뀌는 수치는 추측 금지. 모르면 모른다고 하고 확인 경로를 안내하라.
- 아래 벤치마크는 "평균 범위" 참고용이며, 사장님께는 항상 "지역·시점에 따라 다름"을 전제로 제시하라.
...
<funding_extraction>
fundingPrograms 필드는 빈 배열([])로 반환하세요 — 서버가 지원사업 SSOT(공고 상태·마감일 포함)
매칭 결과로 대체합니다. 프로그램명·금액·자격을 생성하지 마세요 (매년 바뀌는 수치라 생성 즉시 낡습니다).
</funding_extraction>
...
9. 한국어로 응답하세요.
10. **상권 분석(marketAnalysis)**: 값을 생성하지 마세요 (score 0·grade "C"·빈 문자열) — 서버가 실측으로 대체합니다.
